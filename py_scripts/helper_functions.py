...
def raw_file_clean(dict_months):
    ...
    for month, url in dict_months.items():
        if month == '2015_01':
            cleaned_df = pd.read_csv(f'{raw_folder}/{raw_filename}.csv', header=1, skiprows=11)
            if cleaned_df.columns[-1] != 'Total':
                cleaned_df = pd.read_csv(f'{raw_folder}/{raw_filename}.csv', header=1, skiprows=10)
            shutil.copyfile(f'{raw_folder}/{raw_filename}.csv', f"{raw_folder_renamed}/{month}_el_iman_sales_report.csv")
            cleaned_df.to_csv(f"{clean_folder_renamed}/{month}_el_iman_sales_report_cleaned.csv", index=False)
        else:
            index += 1
            cleaned_df = pd.read_csv(f'{raw_folder}/{raw_filename} ({index}).csv', header=1, skiprows=11)
            if cleaned_df.columns[-1] != 'Total':
                cleaned_df = pd.read_csv(f'{raw_folder}/{raw_filename} ({index}).csv', header=1, skiprows=10)
            shutil.copyfile(f'{raw_folder}/{raw_filename} ({index}).csv', f"{raw_folder_renamed}/{month}_el_iman_sales_report.csv")        
            cleaned_df.to_csv(f"{clean_folder_renamed}/{month}_el_iman_sales_report_cleaned.csv", index=False)
